...
#[macro_export]
macro_rules! bulk {
    (apply, $input:ty, $output:ty) => {
        $crate::bulk!(apply, bulk_apply, $input, $output);
    };
    (cost, $input:ty, $output:ty) => {
        $crate::bulk!(cost, bulk_cost, $input, $output);
    };
    (gradient, $input:ty, $output:ty) => {
        $crate::bulk!(gradient, bulk_gradient, $input, $output);
    };
    (hessian, $input:ty, $output:ty) => {
        $crate::bulk!(hessian, bulk_hessian, $input, $output);
    };
    (jacobian, $input:ty, $output:ty) => {
        $crate::bulk!(jacobian, bulk_jacobian, $input, $output);
    };
    ($method_name:ident, $bulk_method_name:ident, $input:ty, $output:ty) => {
        #[doc = concat!(
            "Compute `",
        ...
        fn $bulk_method_name<P>(&self, params: &[P]) -> Result<Vec<$output>, Error>
        where
...
#[cfg(test)]
mod bulk_compatibility_tests {
    use crate::core::{Error, SendAlias, SyncAlias};
    #[cfg(feature = "rayon")]
    use rayon::prelude::*;
    ...
    #[test]
    fn three_argument_bulk_invocation_remains_compatible() {
