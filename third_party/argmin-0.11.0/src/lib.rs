...
// `wasm-bindgen` configures getrandom's Web backend. This explicit reference
// records that the direct dependency is intentional even though argmin obtains
// entropy through rand rather than calling getrandom itself.
