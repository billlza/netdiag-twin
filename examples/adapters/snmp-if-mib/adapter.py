...
def preflight_report(args: argparse.Namespace) -> dict:
    ...
    if mode == "sample":
        input_ok, input_message = True, "built-in sample counters"
    else:
...
def load_interfaces(path: Path) -> list[dict[str, object]]:
    try:
        raw = json.loads(
    ...
    if isinstance(raw, list):
        ...
        rows = raw.get("interfaces", [raw])
        if not isinstance(rows, list):
            raise ValueError("input JSON 'interfaces' must be an array")
    else:
        raise ValueError("input JSON must be an object or an array of objects")
    if not rows:
        raise ValueError("input JSON must contain at least one interface")
    if len(rows) > MAX_INTERFACES:
        raise ValueError(f"input JSON exceeds the {MAX_INTERFACES}-interface limit")
    for index, row in enumerate(rows, start=1):
        if not isinstance(row, dict) or not row:
            raise ValueError(f"interface {index} must be a non-empty object")
    return rows
...
def finite_sum(name: str, *values: float) -> float:
    return finite_value(name, sum(values))
...
def record_from_if_mib(row: dict[str, object]) -> dict:
    interval = required_number(row, "interval_seconds")
    if interval == 0.0:
        raise ValueError("interval_seconds must be greater than zero")
    octets = finite_sum(
        "octet count",
        required_number(row, "if_in_octets_delta"),
        required_number(row, "if_out_octets_delta"),
    )
    throughput_mbps = finite_value(
        "throughput_mbps", (octets / interval) * 8.0 / 1_000_000.0
    )
    packets = finite_sum(
        "packet count",
        required_number(row, "if_in_ucast_pkts_delta"),
        required_number(row, "if_out_ucast_pkts_delta"),
    )
    discards = finite_sum(
        "discard count",
        required_number(row, "if_in_discards_delta"),
        required_number(row, "if_out_discards_delta"),
    )
    errors = finite_sum(
        "error count",
        required_number(row, "if_in_errors_delta"),
        required_number(row, "if_out_errors_delta"),
    )
    ...
    return {
        ...
        "latency_ms": 0.0,
        "jitter_ms": 0.0,
        "packet_loss_rate": percentage("packet_loss_rate", discards, denominator),
        "retransmission_rate": percentage(
            "retransmission_rate", errors, denominator
        ),
        "timeout_events": 0.0,
        "retry_events": errors,
...
def validate_live_input(path: Path | None) -> tuple[bool, str]:
    ...
    for index, row in enumerate(rows, start=1):
        try:
            record_from_if_mib(row)
        except ValueError as error:
            return False, f"interface {index}: {error}"
    return True, f"{path} ({len(rows)} interfaces)"
...
def main() -> None:
    ...
    if args.emit_sample:
        ...
        rows = load_interfaces(args.input_json)
    else:
    ...
    for index, row in enumerate(rows, start=1):
        try:
            records.append(record_from_if_mib(row))
        except ValueError as error:
            raise ValueError(f"interface {index}: {error}") from error
    print(
        json.dumps(
            {
