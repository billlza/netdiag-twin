...
def parse_resolution_output(
    ...
    if (
        ...
        raise ProbeRuntimeError("resolver worker returned an invalid response")
    if status in {RESOLUTION_FAILURE, NETWORK_FAILURE}:
...
def bounded_resolve(
    ...
    command = resolver_command(host, port, socket_type)
    try:
...
def dns_attempt(host: str, timeout_seconds: float) -> ProbeAttempt:
    started = time.monotonic()
...
def live_payload(args: argparse.Namespace, host: str) -> dict:
    ...
    attempts = [dns_attempt(host, args.timeout_secs) for _ in range(args.count)]
