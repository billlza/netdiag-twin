...
CERTIFICATE_FAILURE = "certificate"
...
def classify_tls_error(error: OSError) -> str:
...
def tls_attempt(
    host: str, port: int, timeout_seconds: float, context: ssl.SSLContext
) -> ProbeAttempt:
    started = time.monotonic()
    ...
    for family, socket_address in resolution.addresses:
        ...
        try:
            ...
            last_failure = classify_tls_error(error)
            if last_failure in {CERTIFICATE_FAILURE, PROTOCOL_FAILURE}:
                break
...
def live_payload(args: argparse.Namespace, host: str) -> dict:
    try:
        ...
        raise ProbeRuntimeError(
            "TLS trust configuration could not be loaded"
        ) from error
    fault_started = datetime.now(timezone.utc)
    attempts = [
        tls_attempt(host, args.port, args.timeout_secs, context)
    ...
    tls_failures = sum(
        failure_counts[kind] for kind in (CERTIFICATE_FAILURE, PROTOCOL_FAILURE)
    )
    return {
        "schema": "netdiag-adapter-payload/v2",
        ...
        "records": [
            {
                ...
                "tls_failure_events": float(tls_failures),
                "quic_blocked_ratio": 0.0,
            }
        ],
        "measurement_quality": MEASUREMENT_QUALITY,
